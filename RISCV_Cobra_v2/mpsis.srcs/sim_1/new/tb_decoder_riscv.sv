...
module tb_decoder_riscv();
  ...
  initial begin
    ...
    for (V=0; V<cycle/10; V=V+1) begin  // illegal ÔÓ 11 ‚ ÍÓÌˆÂ opcode
        instr[1:0]  = $random;
        instr[6:2]  = {1'b0,V[1:0],2'b0};
        instr[31:7] = 'b0;
        #delay;
    end
    for (V=0; V<cycle; V=V+1) begin  // illegal ÔÓ OP_OPCODE funct7
    ...
    for (V=0; V<cycle; V=V+1) begin  // illegal ÔÓ SYSTEM_OPCODE
